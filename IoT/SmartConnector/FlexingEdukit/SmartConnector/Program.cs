    ...
    class Program
    {
        ...
        public class Service
        {
            ...
            public Task<Boolean> Connect()
            {
                ...
                {
                    //192.168.0.122 : 2004
                    string ip = edgeConfigResult.EdukitIP;
                    int port = Int32.Parse(edgeConfigResult.EdukitPort);
                    ...
                    Console.WriteLine("##########################");
                    // Console.WriteLine(edgeConfigResult); // edgeConfigResult = EdgeConfigFile.json 과 동일
            ...
            private void client_MqttMsgPublishReceived(object sender, MqttMsgPublishEventArgs e)
            {
                ...
                Console.WriteLine(" ReceivedMessage = " + ReceivedMessage); // {"tagId":"1", "value":"1"}
            ...
            private void XGTWrite(XGTClass xGTClass, string ReceivedMessage)
            {
                ...
                // 읽어올 데이터를 XGTAddressData로 생성xGTClass.Connect(edgeConfigResult.EdukitIP, 2004);
                ...
                else if (test.tagId == "14") // 1호기 Delay
                {
                    if (test.value == "0")
                    {
                ...
                else if (test.tagId == "31") // 2호기 색 선별 0: 색 선별, 1: 모두
                {
                    if (test.value == "0")
                    {
                ...
                else if (test.tagId == "35") // 비상정지 (비상정지 시: 0 평소: 1) 
                {
                    if (test.value == "0")
                    {
                ...
                else if (test.tagId == "36") // 자재투입LIMIT 최대5
                {
                    if (test.value == "0")
                    {
                ...
                else if (test.tagId == "37") // DiceValue : vision.py를 통해 받는 값
                {
                    if (test.value == "0")
                    {
                ...
                else if (test.tagId == "38") // 1호기 Delay
                {
                    if (test.value == "0")
                    {
            ...
            private void ConnectionStart(int DelayTime, XGTClass xGTClass, string ip, int port)
            {
                ...
                // Console.WriteLine(edgeConfigResult.EdukitId);
                ServerSocket.Emit("joinRoom", edgeConfigResult.EdukitId);
                // Console.WriteLine("YYYYYYYYYYYYYYYYYYYYYYYYYYY");
                ...
                Start.Address = "0";        // 메모리 주소중 숫자만 입력 (Ex) M00000이면 0만 입력
                Start.Name = "Start";
                Start.TagId = "1";
                ...
                */
                ...
                // WordAddressList.Add(no3_motor1_point, "K");
                // WordAddressList.Add(no3_motor1_velocity, "K");
                                ...
                                if (val.DataList != null)
                                {
                                    if (val.DataList[0].IntData == 0)
                                    {
                                        EdukitNewdata newdata = new EdukitNewdata
                                        {
                                            ...
                                            value = val.DataList[0].IntData.ToString() // false
                                        };
                                    ...
                                    else
                                    {
                                        EdukitNewdata newdata = new EdukitNewdata
                                        {
                                            name = address.Key.Name,
                                            tagId = address.Key.TagId,
                                            value = val.DataList[0].IntData.ToString() //true
                                        };
                                        edukitData.Add(newdata);
                                    }
                                }
      ...
      */
                        ...
                        {
                            // Console.Clear();
                            Console.WriteLine("DebugType=Debug");
                    ...
                    // EdgeConfigFile.json 의 DelayTime
                    Thread.Sleep(DelayTime);
                }
            }

            static Task<string> SocketIoData(List<EdukitNewdata> EduKitData) //socket 송신 함수
            {
            ...
            static void MqttData(MqttData EduKitData) //mqtt 송신 함수
            {
